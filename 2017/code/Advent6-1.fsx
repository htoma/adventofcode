...
let rec distribute (values: int array) (memories: string list) count =
    let hash = values |> buildMemory
    if memories |> List.contains hash then count
    else
        ...
        distribute newValues (hash :: memories) (count + 1)

let line = IO.File.ReadAllLines(@"2017\data\Advent6-1.txt").[0]
